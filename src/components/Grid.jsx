    ...
    <div className="max-w-[1600px] mx-auto px-6 py-6 overflow-x-auto">
      <div className="grid gap-2 min-w-fit" style={{ gridTemplateColumns: `200px repeat(${sortedSemesters.length}, minmax(160px, 1fr))` }}>
        <div className="flex items-center justify-center">
          <button onClick={onAddSemester} className="w-10 h-10 rounded-md flex items-center justify-center hover:scale-105 transition" style={{ border: '1.5px solid var(--maroon)', color: 'var(--maroon)' }} title="학기 추가">
            <Plus size={18} />
          </button>
        </div>
        ...
        {sortedSemesters.map((s) => (
          <div key={s.id} className="semester-header group">
            <span>{s.label}</span>
            <button onClick={() => onRemoveSemester(s.id)} className="opacity-0 group-hover:opacity-100 transition" title="학기 삭제" style={{ color: 'var(--maroon)' }}><X size={14} /></button>
          </div>
        ))}
              ...
              <div className="category-label group relative">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                    <button onClick={() => onEditCategory(cat)} title="수정" style={{ color: 'var(--ink-soft)' }}><Edit3 size={13} /></button>
                    <button onClick={() => onRemoveCategory(cat.id)} title="삭제" style={{ color: 'var(--maroon)' }}><Trash2 size={13} /></button>
                  </div>
                </div>
                <div className="text-xs mt-1" style={{ color: overflowing ? '#5B6B23' : 'var(--ink-soft)' }}>{earned}/{cat.requiredCredits} 학점</div>
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${pct}%`, background: overflowing ? '#5B6B23' : 'var(--maroon)' }} />
                </div>
                {cat.tag && (
                  <div className="mt-2">
                    <span className="tag-badge" style={{ background: TAG_COLORS[cat.tag]?.bg || 'var(--maroon)', color: TAG_COLORS[cat.tag]?.text || 'var(--cream)' }}>{cat.tag}</span>
                  </div>
                )}
              </div>
        ...
        <div className="flex items-center justify-center">
          <button onClick={onAddCategory} className="w-10 h-10 rounded-md flex items-center justify-center hover:scale-105 transition" style={{ border: '1.5px solid var(--maroon)', color: 'var(--maroon)' }} title="카테고리 추가">
            <Plus size={18} />
          </button>
        </div>
        {sortedSemesters.map((s) => {
          ...
          return (
            <div key={s.id} className="summary-cell">
              ...
              <div className="text-xs" style={{ color: 'var(--ink-soft)' }}>{stat.credits} 학점</div>
            </div>
          )
        })}
      </div>
