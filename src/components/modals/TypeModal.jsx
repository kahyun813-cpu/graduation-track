...
export default function TypeModal({ type, onSave, onDelete, onClose, deletable }) {
  ...
  const save = () => {
    if (!name.trim()) { alert('유형 이름을 입력해주세요.'); return }
    onSave({ id: type.id, name: name.trim(), color: customMode ? customColor : color })
  }

  return (
    <ModalShell title={type._isNew ? '유형 추가' : '유형 수정'} onClose={onClose}>
      <div className="space-y-4">
        <Field label="이름">
          <input value={name} onChange={(e) => setName(e.target.value)} autoFocus onKeyDown={(e) => { if (e.key === 'Enter') save() }} className="text-input" placeholder="예: 전공, 교양…" />
        </Field>
        ...
        <Field label="색상">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-2 text-xs cursor-pointer" style={{ color: 'var(--ink-soft)' }}><input type="checkbox" checked={customMode} onChange={(e) => setCustomMode(e.target.checked)} /> 직접 입력</label>
            {customMode && (
              <>
